...
impl<T: Clone + Integer + CheckedAdd + CheckedMul> Iterator for ContinuedFraction<T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let a2 = d.checked_mul(&self.a1.checked_add(&n.checked_mul(&self.a0)?)?)?;
        let b2 = d.checked_mul(&self.b1.checked_add(&n.checked_mul(&self.b0)?)?)?;
...
/// Produce the convergents of a simple continued fraction given an integer sequence representing the partial numerators.
...
impl<T: Clone + Integer + CheckedAdd + CheckedMul> Iterator for SimpleContinuedFraction<T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let a2 = self.a1.checked_add(&n.checked_mul(&self.a0)?)?;
        let b2 = self.b1.checked_add(&n.checked_mul(&self.b0)?)?;
