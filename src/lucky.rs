...
impl Iterator for Lucky<Number> {
    type Item = Number;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if self.terms.is_empty() {
            ...
            self.terms.push([self.odds.next()?, 2]);
            self.ctr.incr()?;
            return out;
        }
...
impl Iterator for Lucky<BigInt> {
    type Item = BigInt;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if self.terms.is_empty() {
            let out = self.odds.next();
            self.terms.push([self.odds.next()?, BigInt::from(2)]);
            self.ctr.incr()?;
            return out;
        }
...
impl Iterator for Unlucky<Number> {
    type Item = Number;
...
impl Iterator for Unlucky<BigInt> {
    type Item = BigInt;
